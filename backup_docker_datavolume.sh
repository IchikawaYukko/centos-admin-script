...
#docker cp $CONTAINER:/temp.tar.bz2 $BACKUP_DIR/docker_data`date +%Y%m%d`.tar.bz2
docker exec -i $CONTAINER cat /temp.tar.bz2|swift upload --object-name docker/docker_data`date +%Y%m%d`.tar.bz2 -S 5368709119 Backup - 1>&2 >> $BACKUP_LOG
